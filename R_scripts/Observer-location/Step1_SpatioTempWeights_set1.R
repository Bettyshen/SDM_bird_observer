#' [Observer-location BRT Model - set 1]
#' 
...
all_birds.weight <- spatiotemp_weights(occ.data = oregon_set1,
                                            samp.events = sample_events_data,
                                            spatial.dist = 104200,
                                            temporal.dist = 76)
write.csv(all_birds.weight, 
          file = sprintf("%s/SurveyCentered_weights_set1.csv", base_dir),
          row.names = FALSE)
...
write.csv(all_birds.weight.env.fixed, 
          file = sprintf("%s/Median_surveyCentered_fixedRadius_weights_set1.csv", base_dir),
          row.names = FALSE)
    
write.csv(all_birds.weight.env.noRadius, 
          file = sprintf("%s/Median_surveyCentered_noRadius_weights_set1.csv", base_dir),
          row.names = FALSE)

write.csv(all_birds.weight.env.ESW,
          file = sprintf("%s/Median_surveyCentered_ESW_weights_set1.csv", base_dir),
          row.names = FALSE)
