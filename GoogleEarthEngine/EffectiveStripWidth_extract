//***************Goal********************//
// The goal of this script is to extract pixel value for all species via 
// observer-location approach from effective strip width method. Effective strip width is species-specific.
...
/////////////////////////////////////////////////////////////////////////////////////////
///Section 3 - Loop over each year and extract pixel value from using species-specific 
// effective strip width extraction strategy
/////////////////////////////////////////////////////////////////////////////////////////
...
//=========================================================================//
// ***** Extraction Method: Using species-specific Effective Radius*******
// Divide species list into 50 as a set to run
//========================================================================//
// Function to print effective strip width for each species 
// Just to make sure we are retrieve the corresponding effective strip width value for each species

function effectiveSW(speciesName){
  // Retrieve species effective strip width value
    var speciesESW = effective_sw.filter(ee.Filter.eq("common_name", speciesName));
    var ESW = speciesESW.aggregate_array("effective_strip_width").get(0);
    print(ESW, speciesName);
}


// Iterate over the speciesList and apply the effectiveSW function to each species
speciesList.evaluate(function(list) {
...
function processSpeciesAndYear(speciesName, year) {
  //Filter bird data for the current species
    ...
    var speciesYearsResults = ee.FeatureCollection([]);
      ...
      // Buffer the points
      var ptsEnvi = yearBird.map(bufferPoints(ESW,false));
...
var allYearsResultsComplete = allYearsResults.merge(allYearsResults2).merge(allYearsResults3).merge(allYearsResults4);
  ...
  description: 'Median_surveyCentered_ESW',
...
var AllSpecialYearResults = allYearsResultsComplete.merge(allSpecialYearsResults).merge(allSpecialYearsResults2).merge(allSpecialYearsResults3).merge(allSpecialYearsResults4);
  ...
  description: 'Median_withSpecialYear_surveyCentered_ESW',
  folder: "GoogleEarthEngine",
  fileFormat: 'CSV'
  });
